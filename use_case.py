...
def hpc(hpc_data: gpd.GeoDataFrame,
    ...
    charging_events = uc_dict['charging_event'].loc[
        uc_dict["charging_event"]["charging_use_case"].isin(["urban_fast"]) & ~uc_dict["charging_event"][
            "location"].isin(["shopping"])].reset_index()
...
def public(
    ...
    uc_id = "public"
    print("Use case: " + uc_id)
    charging_events = uc_dict["charging_event"].reset_index()
...
def home(
    ...
    if mode == "apartment":
        ...
        print("Use case: " + uc_id)
        charging_events = uc_dict["charging_event"].loc[
            uc_dict["charging_event"]["charging_use_case"].isin(["home_apartment"])].reset_index()
    elif mode == "detached":
        uc_id = "home_detached"
        print("Use case: " + uc_id)
        charging_events = uc_dict["charging_event"].loc[
            uc_dict["charging_event"]["charging_use_case"].isin(["home_detached"])].reset_index()
    else:
        print("wrong mode")
...
def work(
    ...
    charging_events = uc_dict["charging_event"].loc[
        uc_dict["charging_event"]["charging_use_case"].isin(["work"])].reset_index()
...
def retail(retail_data: gpd.GeoDataFrame,
    ...
    charging_events_retail_hpc = uc_dict['charging_event'].loc[
        uc_dict["charging_event"]["charging_use_case"].isin(["urban_fast"]) & uc_dict["charging_event"][
            "location"].isin(["shopping"])]

    charging_events = pd.concat([charging_events_retail_slow, charging_events_retail_hpc], axis=0,
